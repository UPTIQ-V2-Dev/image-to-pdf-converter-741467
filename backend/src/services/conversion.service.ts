...
import { ConversionJob } from '../generated/prisma/index.js';
 ...
 */
const createConversionJob = async (userId: number, settings: ConversionSettings): Promise<ConversionJob> => {
 ...
 */
const addImagesToConversionJob = async (
    conversionId: string,
    userId: number,
    images: ImageUpload[]
): Promise<UploadUrl[]> => {
 ...
 */
const startConversionProcessing = async (conversionId: string, userId: number) => {
    // Verify user owns the conversion job
    const conversionJob = await prisma.conversionJob.findFirst({
        where: { id: conversionId, userId },
        include: { images: true }
 ...
 * @param {string} conversionId - Conversion job ID
 */
const processConversionInBackground = async (conversionId: string) => {
    try {
 ...
 */
const updateUserConversionHistory = async (conversionId: string, success: boolean = true) => {
 ...
 */
const getConversionJobById = async (conversionId: string, userId: number) => {
    const conversionJob = await prisma.conversionJob.findFirst({
        where: { id: conversionId, userId },
        include: { images: true }
    });
 ...
 */
const downloadConvertedPDF = async (conversionId: string, userId: number) => {
    const conversionJob = await prisma.conversionJob.findFirst({
        where: { id: conversionId, userId }
    });
 ...
 * @param {number} userId - User ID
 */
const deleteConversionJob = async (conversionId: string, userId: number): Promise<void> => {
    const conversionJob = await prisma.conversionJob.findFirst({
        where: { id: conversionId, userId },
        include: { images: true }
    });
 ...
 */
const queryConversionJobs = async (
    userId: number,
    filter: any,
    options: {
        limit?: number;
 ...
 */
const getConversionProgress = async (conversionId: string, userId: number): Promise<ConversionProgress> => {
    const conversionJob = await prisma.conversionJob.findFirst({
        where: { id: conversionId, userId },
        include: { images: true }
    });
